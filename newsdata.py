...
from newsdatadb import get_most_popular_three_articles, get_most_popular_author, get_date_when_error_test_failed
...
def find_top_three_articles():
  ...
  posts = "".join(POST % (path[9:], sum) for path, sum in get_most_popular_three_articles())
...
def find_best_authors():
  ...
  posts = "".join(POST2 % (author, views) for author, views in get_most_popular_author())
  html = HTML_WRAP % ("",posts,"")
  return html  
...
def find_errors():
  ...
  posts = "".join(POST3 % (date, format(percentage_error,'.2f')+'%') for date, percentage_error in get_date_when_error_test_failed())
  html = HTML_WRAP % ("","",posts)
  return html  
